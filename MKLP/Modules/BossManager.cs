    ...
    public static class BossManager
    {
        ...
        public static async void Hooks_OnNPCSpawn(NpcSpawnEventArgs args)
        {
            ...
            for (int i = 0; i < Main.maxNPCs; i++)
            {
                ...
                {
                    case BossManager.BossAllowType.Allowed:
                        break;
                    case BossManager.BossAllowType.NotAllowed:
                        {
                            TShock.Utils.Broadcast(MKLP.GetText($"{npc.FullName} isn't allowed yet!"), Color.MediumPurple);
                            DespawnNPC();
                            break;
                        }
                    case BossManager.BossAllowType.LackOfPlayers:
                        {
                            TShock.Utils.Broadcast(MKLP.GetText($"{npc.FullName} required {requireplayers} Players to be summoned"), Color.MediumPurple);
                            DespawnNPC();
                            break;
                        }
                    case BossManager.BossAllowType.Illegal:
                        {
                            DespawnNPC();
                            break;
                        }

                    case BossManager.BossAllowType.MechDusaNotAllowed1:
                        {
                    ...
                    case BossManager.BossAllowType.MechDusaNotAllowed2:
                        {
                            await Task.Run(async () => {
                                if (!NPC_Is_Active(MechdusaIDs))
                                {
